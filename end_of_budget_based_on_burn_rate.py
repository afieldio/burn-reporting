...
def main():
	spent_to_date = input("What is spent to date (excl PM in Days)? ")
	start_date_input = input("What is the start date? ")
	total_budget = input("What is the total budget? (in Days)")
	end_date = today

	start_date = datetime(*start_date_input[:4])
	# end_date = datetime(*end_date_input[:4])
	num_days = workdaysub(start_date, end_date)
	burn_rate = spent_to_date/num_days
	print "Burn Rate: %s" % burn_rate
	num_days_at_given_burn = total_budget/burn_rate

	expected_end_date = workdayadd(start_date, int(num_days_at_given_burn))
